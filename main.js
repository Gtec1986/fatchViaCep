...
const cep = document.querySelector('#cep').onkeyup = e => {
     function search() {
          fetch(`https://viacep.com.br/ws/${CEP.value}/json`)
               ...
               .then(data => {
                    console.log(data); for (let d in data) {
                         n_data[i] = data[d];
                         form[i].value = n_data[i]; i = i + 1
                    }
                    for (i = 0; i < complements.length; i++) {
                         if (CEP.value === 'true' || CEP.value === 'não Encontrado') { form[i].value = 'não Encontrado' }
                    }
               })

               .catch(err => { if (form.value !== undefined) { console.log('deu erro') } })
